function rankc_opt_dl1(file_actual,n,s_start,s_end)
...
for k=k_start:k_end
    ...
    [complete_rank unsort_score]=rankcentrality_dl(file);
    complete_rank
    for i = 1:n
        for j = 1:n
            if( i ~= j)
    ...
    for s_no = s_start:s_end
        ...
        innersum =0;
        for iter= 1:n
            innersum = innersum+abs(complete_rank(iter)-find(sample_rank == complete_rank(iter)));
        end
        dl_err(vec_len) = innersum/n;
        vec_len = vec_len+1;

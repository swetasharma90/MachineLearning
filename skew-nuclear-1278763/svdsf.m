function [U,S,V,flag] = svdsf(varargin)
...
% Modified by David F. Gleich to compute for functions

Af = varargin{1};
Atf = varargin{2};
mn = varargin{3}; m = mn(1); n = mn(2);
p = min(m,n);
q = max(m,n);
...
[W,D,bflag] = eigs(@svdmatvec,m+n,bk,bsigma,boptions,m,Af,Atf,varargin{7:end});

if ~isreal(D) || (~isreal(W))
...
function y=svdmatvec(x,m,A,At,varargin)
x1 = x(1:m);
x2 = x(m+1:end);
y = [A(x2,varargin{:});  At(x1,varargin{:})];

function nnm_opt_dl1_btl(n,s_start,s_end)
...
for k=k_start:k_end
    ...
    [complete_rank unsort_score]=nnm_dl(file);
    complete_rank
    for i = 1:n
        for j = 1:n
            if( i ~= j)
    ...
    for s_no = s_start:s_end
        ...
        [sample_rank score]=nnm_dl(filename);
        sample_rank
        innersum =0;
        for iter= 1:n
            innersum = innersum+abs(complete_rank(iter)-sample_rank(iter));
